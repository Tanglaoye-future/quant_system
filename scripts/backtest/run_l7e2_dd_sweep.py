#!/usr/bin/env python3
"""
equity_factor L7-E v2: 探索 DD 改善变体.

L7-E winner (atr_stop 1.5 / atr_target 3.0 / max_hold 40) 把 Sharpe 0.23 → 0.41,
但 DD 仍 -19.9% (用户套牢痛点). 这一轮专攻 DD:

E (基准): atr_stop=1.5 / atr_target=3.0 / max_hold=40
E2 (更紧止损): atr_stop=1.2
E3 (短持仓): max_hold=30
E4 (regime exit): + m5_regime_exit_enabled=true (HS300 < MA60 强制平仓)
E5 (partial exit): + partial_exit_enabled=true (50% TP 锁利)

a_share HS300, 2022-2026, 5 个实验 × 35min = ~3h.
"""
...
E_BASE = {"atr_stop_mult": 1.5, "atr_target_mult": 3.0, "max_hold_days": 40}

EXPERIMENTS: list[tuple[str, dict]] = [
    ("L7-E-base",        E_BASE),
    ("L7-E2-stop12",     {**E_BASE, "atr_stop_mult": 1.2}),
    ("L7-E3-hold30",     {**E_BASE, "max_hold_days": 30}),
    ("L7-E4-regime-exit",{**E_BASE, "m5_regime_exit_enabled": True}),
    ("L7-E5-partial",    {**E_BASE, "partial_exit_enabled": True, "partial_exit_pct": 0.5}),
]
...
def run_one(tag, overrides, base_raw, market):
    ...
    out_dir = ROOT / f"data/backtest/_l7e2_{tag}"
    if out_dir.exists():
...
def main():
    ...
    out_md = ROOT / "data/backtest/equity_factor_l7e2_dd_summary.md"
    lines = [
        "# equity_factor L7-E v2 DD 优化 sweep",
        "",
        f"市场: {MARKET}  窗口: {START} → {END}",
        "",
        "E winner (atr_stop=1.5/atr_target=3.0/max_hold=40) Sharpe 0.41 但 DD -19.9%.",
        "本轮探索: E2 (更紧止损) / E3 (短持仓) / E4 (regime exit) / E5 (partial exit)",
        "",
    ...
    print(f"\n[L7-E v2 sweep] summary → {out_md}", flush=True)
