#!/usr/bin/env python3
"""
equity_factor L7-E v3 组合 sweep.

L7-E v2 发现:
- E4 (regime_exit): DD -19.5% → -14.0% (唯一显著改善 DD)
- E3 (hold30): Sharpe 0.226 → 0.538 (最高)
- E5 (partial_exit): win rate +13pp (从 36% → 50%)

本轮叠加找最优组合:
  C1: E base (atr_stop=1.5 / atr_target=3.0 / max_hold=40) + regime_exit + hold30
  C2: E base + regime_exit + partial_exit
  C3: E base + regime_exit + hold30 + partial_exit (all)
"""
...
EXPERIMENTS = [
    ("L7-C1-regime-hold30", {**E_BASE, "m5_regime_exit_enabled": True,
                              "max_hold_days": 30}),
    ("L7-C2-regime-partial",{**E_BASE, "m5_regime_exit_enabled": True,
                              "partial_exit_enabled": True, "partial_exit_pct": 0.5}),
    ("L7-C3-all",           {**E_BASE, "m5_regime_exit_enabled": True,
                              "max_hold_days": 30,
                              "partial_exit_enabled": True, "partial_exit_pct": 0.5}),
]
...
def run_one(tag, overrides, base_raw, market):
    ...
    out_dir = ROOT / f"data/backtest/_l7e3_{tag}"
    if out_dir.exists():
...
def main():
    ...
    out_md = ROOT / "data/backtest/equity_factor_l7e3_combo_summary.md"
    lines = ["# equity_factor L7-E v3 组合 sweep", "",
             f"市场: {MARKET}  窗口: {START} → {END}", "",
             "C1=E+regime+hold30 / C2=E+regime+partial / C3=all", "",
    ...
    print(f"\n[L7-E v3 combo sweep] summary → {out_md}", flush=True)
