#!/usr/bin/env python3
"""
equity_factor L8 实验：rev_accel 因子 + 北向资金 RSI 带（A 股）。

baseline = 当前实盘配置 (L7-C3 + fcf_yield 0.20)。
4 个对照（4y 窗口 2022-01-01 → 2026-05-04，与 L7 一致）：

  L8-base       不动 yaml（基准）
  L8A-rev_accel  +rev_accel 0.10，rev_growth 0.15→0.10（factors.weights）
  L8B-north_top10 +northbound widen (threshold=2.0，照搬 HK v14 严格档)
  L8C-combo      A + B

支持同时 override timing 与 weights — L7 verify_8y harness 仅 timing，本脚本扩展。
"""
...
EXPERIMENTS: list[tuple[str, dict, dict]] = [
    ...
    ("L8B-north_top10", {
        ...
        "m3_southbound_threshold": 2.0,        # 仅 top 10% 强买日触发
        "m3_southbound_widen_lo_pts": 3.0,
        "m3_southbound_widen_hi_pts": 3.0,
        "m3_southbound_vol_relax": 0.2,
    }, {}),

    ("L8C-combo", {
        "m3_southbound_widen_enabled": True,
        "m3_southbound_ma_window": 20,
        "m3_southbound_threshold": 2.0,
        "m3_southbound_widen_lo_pts": 3.0,
        "m3_southbound_widen_hi_pts": 3.0,
        "m3_southbound_vol_relax": 0.2,
    }, {
...
def run_one(tag: str, timing_override: dict, weights_override: dict,
    ...
    out_dir = ROOT / f"data/backtest/_l8_{tag}"
    if out_dir.exists():
...
def main():
    ...
    out_md = ROOT / "data/backtest/equity_factor_l8_factor_flow_summary.md"
    lines = [
        "# equity_factor L8: rev_accel + 北向资金 (A 股)", "",
    ...
    print(f"\n[L8 sweep] summary → {out_md}", flush=True)
