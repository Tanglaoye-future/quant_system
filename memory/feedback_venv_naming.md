...
description: 本仓 Python venv 必须用 venv/ 不带 dot 前缀；macOS UF_HIDDEN + Python 3.14 site.py 组合会让 .venv/ 下所有 .pth 失效
metadata:
  type: feedback
---
...
- 如有人新机器 clone 后习惯性 `python3 -m venv .venv`，会立即遇到 import 失败 —— 不要 workaround（`PYTHONPATH=src` / `chflags`），直接 `rm -rf .venv && python3 -m venv venv` 重建
