...
export default function App() {
  const { data, markets, matrix, panicData, loading, error, updatedAt, refresh } = useReportData();
  ...
  return (
    <Layout date={date} updatedAt={updatedAt} onRefresh={refresh} loading={loading}>
      ...
      {data && markets && <DashboardPage data={data} markets={markets} matrix={matrix} panicData={panicData} />}
    </Layout>
  );
}
