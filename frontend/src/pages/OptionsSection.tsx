...
export default function OptionsSection({ data }: { data: OptionsData }) {
  if (data._missing) {
    return (
      <section className="mb-12 animate-fadeIn">
        <h2 className="text-xl font-semibold tracking-tight mb-4">QQQ 期权套利</h2>
        <GlassCard>
  ...
  return (
    <section className="mb-12 animate-fadeIn">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold tracking-tight">QQQ 期权套利</h2>
      ...
      <GlassCard className="mb-4">
        <MetricGrid>
          ...
          <MetricCard label="信号评级" value={data.signal_grade ?? '—'} sub={data.qqq_bullish ? '✅ 看涨' : '❌ 不足'} colorClass={gradeColor} />
        </MetricGrid>
      </GlassCard>

      <GlassCard>
        <div className="text-sm font-semibold text-[#1d1d1f] mb-3">信号详情</div>
        ...
        {signal && (
          <div className="mt-4 bg-[#f5f5f7] rounded-xl p-4 border border-[#e5e5ea]">
            <div className="text-sm font-semibold text-[#30d158] mb-2">
              📋 今日信号：{signal.type}
            </div>
